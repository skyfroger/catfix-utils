import { cloneSpriteRE, mouseInteractionRE, roundVarsRE, setVarsRE, videoInteractionRE, } from "./searchPatterns";
import { validScriptsCount, opcodeCount, opcodeCountArray, filterBlocksByOpcode, } from "./utils";
...
function parallelismGrader(jsonProject, project) {
    ...
    */
    const keyPressedBlocks = filterBlocksByOpcode(jsonProject, "event_whenkeypressed", (b) => {
        var _a;
        const fields = b.fields;
        const hasOperands = "KEY_OPTION" in fields && ((_a = fields === null || fields === void 0 ? void 0 : fields.KEY_OPTION) === null || _a === void 0 ? void 0 : _a[0]) !== null;
        return hasOperands;
    });
    ...
    const keysUsed = keyPressedBlocks
        ...
        var _a;
        const fields = b.fields;
        return (_a = fields === null || fields === void 0 ? void 0 : fields.KEY_OPTION) === null || _a === void 0 ? void 0 : _a[0];
    })
        .sort();
    // запускает ли одна клавиша больше одного скрипта
    const isOneKeyFireMultipleScripts = keysUsed.length > new Set(keysUsed).size;
    ...
    */
    const isBackdropSwitching = opcodeCount(jsonProject, "event_whenbackdropswitchesto") > 1;
    /*
    Запускается ли больше одного скрипта по смене громкости
    */
    const isLoudnessChanging = opcodeCount(jsonProject, "event_whengreaterthan", (b) => {
        var _a;
        const fields = b.fields;
        const hasLoudnessField = "WHENGREATERTHANMENU" in fields &&
            ((_a = fields === null || fields === void 0 ? void 0 : fields.WHENGREATERTHANMENU) === null || _a === void 0 ? void 0 : _a[0]) === "LOUDNESS";
    ...
    project.broadcasts.forEach((message) => {
        const currentBroadcastBlocks = filterBlocksByOpcode(jsonProject, "event_whenbroadcastreceived", (b) => {
            var _a;
            const fields = b.fields;
            const hasMessage = "BROADCAST_OPTION" in fields &&
                ((_a = fields === null || fields === void 0 ? void 0 : fields.BROADCAST_OPTION) === null || _a === void 0 ? void 0 : _a[0]) === message;
            return hasMessage;
        });
...
function syncGrader(jsonProject, project) {
    let g = {
        grade: gradesEnum.zero,
        maxGrade: gradesEnum.three,
    };
    ...
    // запуск по сообщению
    const isBroadcastReceived = opcodeCount(jsonProject, "event_whenbroadcastreceived") > 0;
    const isBroadcast = opcodeCount(jsonProject, "event_broadcast") > 0;
    const isBroadcastAndWait = opcodeCount(jsonProject, "event_broadcastandwait") > 0;
    if (isBroadcastReceived && (isBroadcast || isBroadcastAndWait)) {
        g.grade = gradesEnum.two;
    }
    // даём 3 балла за блок Ждать до и обработку события смены фона
    const isWaiUntiltUsed = opcodeCount(jsonProject, "control_wait_until", (b) => {
        const inputs = b.inputs;
        const hasCondition = "CONDITION" in inputs;
        return hasCondition;
    }) > 0;
    const isBackdropChangedUsed = opcodeCount(jsonProject, "event_whenbackdropswitchesto") > 0;
