...
export const ifThenElseRE = new RegExp("if \\<(.+)\\> then\\n(.+\\n)+else\\n(.+\\n)+end");
...
export const roundVarsRE = new RegExp("[^to ]\\((.)+::variables\\)|change \\[(.)+\\] by \\(.+\\)");
// поиск скриптов, которые запускаются по нажатию на клавишу
// с помощью группы получаем клавиши
export const scriptsWithKeyPressEvent = new RegExp("when \\[(.+ v)\\] key pressed::event", "g");
...
export const waitThinkSayRE = new RegExp("(say|think) \\(.+\\) for \\(.+\\) seconds|wait \\(.+\\) seconds");
// синхронизация через ожидание или смену фона
export const waitCondAndBackdropRE = new RegExp("wait until <.+>\\n|when backdrop switches to \\[.+\\]\\n");
