 ...
 */
export function escapeSB(text, onlyRight = true) {
    return onlyRight
        ? text.replace(/([\]\)>])/g, `\\$&`)
        : text.replace(/([\]\)><\(\[])/g, `\\$&`);
}
function parseTarget(sprite) {
    /**
    ...
    const spriteCapIDs = Object.keys(sprite.blocks).filter((key) => HAT_BLOCKS.includes(sprite.blocks[key].opcode));
    // получаем и сохраняем список имён блоков, описанных пользователем
    const customBlockIDs = Object.keys(sprite.blocks).filter((key) => sprite.blocks[key].opcode === PROCEDURES_PROTOTYPE);
    ...
    spriteCapIDs.forEach((hat) => {
        ...
        try {
            const script = toScratchblocks(hat, sprite.blocks, "en", {
                tab: "  ",
            ...
            if (script.includes("\n")) {
                ...
                const w = median(linesCount.map((line) => {
                    return line.length;
                })) * 5;
                parsedSprite.coords.push({ x: x !== null && x !== void 0 ? x : 0, y: y !== null && y !== void 0 ? y : 0, w: w, h: h });
            }
        }
        catch (e) {
            /*
            ...
            такие скрипты
            */
            console.warn(hat, "Скорее всего в скрипте есть пустой блок, который не получается обработать. Весь скрипт пропускается!");
        }
    });
    // объединяем скрипти спрайта в одну строку
    parsedSprite.allScripts = parsedSprite.scripts.reduce((previousValue, currentValue) => {
        return previousValue + currentValue + "\n\n";
    }, "");
    // есть ли комментарии в этом спрайте
    // получаем массив ключей блоков, для которых есть комментарии в json
    let commentedBlocksId = [];
...
function parseProject(scratchProject) {
    /**
    ...
    const stage = scratchProject.targets.filter((t) => t.isStage)[0];
    project.stage = parseTarget(stage);
    // парсим спрайты проекта
    const targets = scratchProject.targets.filter((t) => !t.isStage);
    targets.forEach((t) => {
    ...
    let scriptsString = project.sprites.reduce((previousValue, currentSprite) => {
 ...
 */
export function sbCode(key, blocks) {
    try {
